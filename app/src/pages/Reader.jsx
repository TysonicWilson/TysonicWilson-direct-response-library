...
export default function Reader() {
  ...
  return <div className="reader-page">
    ...
    <div className={`reader-layout reader-layout-v2 ${tocOpen ? "toc-open" : "toc-closed"}`}><aside className="reader-sidebar"><button className="toc-toggle" onClick={() => setTocOpen((value) => !value)}>{tocOpen ? "Hide contents" : "Show contents"}</button>{tocOpen && <nav aria-label="Promotion contents">{tocGroups.map((group) => <div className="toc-group" key={group.label}><span>{group.label}</span><ul className="toc-list">{group.items.map((heading) => <li key={heading.slug}><a href={`#${heading.slug}`}>{heading.raw}</a></li>)}</ul></div>)}</nav>}</aside><main className="reader-main"><div className="reader-toolbar"><div className="mode-switch" role="group" aria-label="Reading mode"><button className={!studyMode ? "active" : ""} onClick={() => setStudyMode(false)}>Read mode</button><button className={studyMode ? "active" : ""} onClick={() => setStudyMode(true)}>Study mode</button></div></div><div className="markdown-body">{renderSections(sections, headingToGroup, studyMode, revealed, (key) => setRevealed((previous) => ({ ...previous, [key]: !previous[key] })))}</div><div className="prevnext-bar">{prevItem ? <Link className="prevnext-link" to={`/library/${encodeURIComponent(prevItem.id)}`}>← {prevItem.id}: {prevItem.title}</Link> : <span />}{nextItem ? <Link className="prevnext-link" to={`/library/${encodeURIComponent(nextItem.id)}`}>{nextItem.id}: {nextItem.title} →</Link> : <span />}</div></main></div>
  </div>;
}
